...
const partsRepeater = (itemsArray = [], collapseView = false) => {
	return itemsArray['parts'].map((part, index) => {
		...
		return <div style={{ width: `${part}%` }} className={`${blockClass} dudesAnimation`}>
			...
			return <div className="f-cmprs-cands__percent-descr fd-f-left fd-craftsmen">
				...
				{isHit
					? <div className="f-text-dark-gray">
						<span>{descriptionText[index]}</span>:<br />
...
const InsufficientData = ({ collapseView, compactMode, dictionary, isRepliesPage }) => {
	if (collapseView) return null

	return <div className={`f-cmprs-cands -insuff -yellow ${ compactMode ? '-compact' : '' }`}>
		<div className="f-cmprs-cands__unsuff-wrap">
			<div className="fd-f-left-middle">
				...
				<span className={compactMode ? 'fd-craftsmen' : 'fd-fat-craftsmen'}>
					...
					{isRepliesPage ? dictionary.titleReplies : dictionary.title}
				</p>
			</div>
		</div>
...
const YetNotAvailable = ({ surveyLink, collapseView, dictionary, compactMode }) => {
	return !collapseView ? <div className={`f-cmprs-cands -yetnot -orange ${ compactMode ? '-compact' : '' }`}>
		<div className="f-cmprs-cands__unsuff-wrap">
			<div className="fd-f-left">
				<img width="30" height="20" style={{marginTop: '15px'}} className="f-cmprs-cands__unsuff-pic" src={`${ruavars.cloudImages}/2017/06/insufficient-get-compare-info.svg`} alt="insufficient-data"/>
				<p className="fd-craftsmen">
					{dictionary.descr}
...
const VacancyIsClosedComponent = ({ collapseView, dictionary, compactMode }) => {
	return !collapseView ? <div style={{ padding: 0 }} className={`f-cmprs-cands -vacclosed ${ compactMode ? '-compact' : '' }`}>
		<p className="fd-craftsmen">
			<span>{dictionary.firstTxt}</span>
			<br/>
			<a href={`${ ukrainian ? '/ua/' : '/' }jobsearch/notepad/vacancies_profile`}>{dictionary.linkDescr}</a>
			<span>{dictionary.lastTxt}</span>
		</p>
...
export default class ComparisonCandidates extends Component {
	render() {
		...
		if(this.props.vacancyIsClosed) {
			return <VacancyIsClosedComponent {...this.props} dictionary={dictionary.vacancyIsClosed} />
		}

		if (this.props.yetNotAvailable) {
			return <YetNotAvailable {...this.props} dictionary={dictionary.yetNotAvailable} />
		}

		if (this.props.showInsufficientData) {
			return <InsufficientData {...this.props} dictionary={dictionary.unsuff} />
		}

		const { salary, experience, collapseView } = this.props

		return <div className={`f-cmprs-cands ${getToggleState(collapseView)}`}>
			<div className="fd-f-between">
				<div className="fd-f1 f-cmprs-cands__salary-block">
					...
					<div className={`percent-descr-left ${collapseView ? '-collapse' : '-expand' }`}>
				...
				<div className="fd-f1 f-cmprs-cands__exp-block">
					...
					<div className={`percent-descr-right ${collapseView ? '-collapse' : '-expand' }`}>
