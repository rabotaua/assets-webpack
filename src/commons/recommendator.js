...
function Recommendator(options) {
    ...
    const helpers = {
        ...
        vacancyLink: function() {
            ...
            if (jQuery('.recommended-vacs-if-zero-result').length > 0) {
                tail = `?ref=recom_new&cre=${ruavars.pageName}&pos=0_search`
            }
            if (jQuery('.bottom-recommended-block').length > 0) {
                tail = `?ref=recom_new&cre=${ruavars.pageName}&pos=keyword`
            }
            if (jQuery('.top-recommended-block').length > 0) {
                tail = `?ref=recom_new&cre=${ruavars.pageName}&pos=without_key`
            }

            return `${uaPrefix}/company${this.notebookId}/vacancy${this.id}${tail}`
        },
